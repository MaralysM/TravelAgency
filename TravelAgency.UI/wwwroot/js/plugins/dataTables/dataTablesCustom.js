            ...
            $.ajax({
                ...
                success: function (retorno) {
                    ...
                    if (retorno.success == null) {
                        ...
                        toastr.error(`No puede eliminar el registro, porque se encuentra relacionado.`, "Error");
                    }
                },
                error: function (response) {
                    toastr.error("Error", `No puede eliminar el registro, porque se encuentra relacionado.`);
                }
            });
        });
}
...
function updateStatus(nameAttribute, source, table, btn) {
    if (table == null)
        table = '#table';
    var link = $(btn);
    var data = {
        id: link.attr(nameAttribute)
    };
    swal({
        title: "Update status",
        text: ` Are you sure to ${$(link).children('span').first().text()}?`,
        type: "warning",
        showCancelButton: true,
        closeOnConfirm: true,
        confirmButtonText: "Yes",
            ...
            $.ajax({
                ...
                success: function (retorno) {
                    if (retorno.success == true) {
                        if ($(link).hasClass('btn-danger')) {
                            $(link).removeClass('btn-success');
                            $(link).addClass('btn-success');
                            $(link).children('i').removeClass('fa-remove');
                            $(link).children('i').addClass('fa-check');
                            $(link).children('span').text('Activate');
                        }
                        else {
                            $(link).removeClass('btn-success');
                            $(link).addClass('btn-success');
                            $(link).children('i').removeClass('fa-check');
                            $(link).children('i').addClass('fa-remove');
                            $(link).children('span').text('Inactivate');
                        }
                        window.location.reload();
                    }
                    else {
                        toastr.error(`An error in the update has occurred. ${retorno.error}`, "Error", { timeOut: 6000 });
                    }
                },
                error: function (response) {
                    toastr.error("Error", `An error in the update has occurred. ${response.responseText}`, "Error", { timeOut: 6000 });
                }
            });
        });
}